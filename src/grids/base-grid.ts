...
abstract class BaseGrid extends Grid {
  ...
  drawBorder() {
    ...
    if (bottom) {
      ...
      this.drawMarkers(
        bottom.values,
        width / (bottom.values.length - len),
        height + 5,
        height,
        bottom.identifier,
        true
    ...
    if (right) {
      ...
      this.drawMarkers(
        right.values,
        height / (right.values.length - len),
        width,
        width + 5,
        right.identifier
      )
  ...
  private drawLabelsValues(
    cType: 'left' | 'right' | 'top' | 'bottom',
    size: number,
    offset: number
  ) {
    const { ctx } = this
    let currentLabel: Label | undefined = this.labels[cType]
    if (currentLabel) {
      const { values, identifier } = currentLabel
      let step = size / values.length
      let currentOffset = step / 2
      if (identifier === 'percent' || identifier === 'value') {
        step = size / (values.length - 1)
        currentOffset = cType === 'top' || cType === 'bottom' ? 0 : 0
      }
      for (let label of values) {
        label = label.toString()
        if (cType === 'left' || cType === 'right') {
          if (this.mainLabel === cType && (label === '0' || label === '0%')) {
            this.y0position = currentOffset
          }
          ctx.textBaseline = 'middle'
          ctx.fillText(label, offset, currentOffset)
        } else {
          if (this.mainLabel === cType && (label === '0' || label === '0%')) {
            this.x0position = currentOffset
          }
          ctx.fillText(label, currentOffset, offset)
        }
        currentOffset += step
      }
    }
